...
<div class="flex row border-secondary rounded-lg ml-3 justify-content-between">
    ...
    <span>
        ...
        <a href="/system-data/eco_typescreate" class="btn btn-info mr-4" role="button">Create</a>
    </span>
    <table class="table table-light table-striped border-secondary rounded-lg mt-2 mr-4">
        ...
        <tbody>
            ...
            <tr>
                <td>{{$env_type->id}}</td>
                <td>{{$env_type->type}}</td>
                <td><a href="/system-data/eco_typesedit/{{$env_type->id}}"class="btn btn-outline-warning" role="button">Edit</a></td>
                <td><button class="btn btn-outline-danger" onclick="event.preventDefault();
                    ...
                    <form id="{{'form-delete-'.$env_type->id}}" style="display:none" method="post" action="/system-data/eco_typesdelete/{{$env_type->id}}">
                        @csrf
                        @method('delete');
                    </form></td>
